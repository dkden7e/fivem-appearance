...
local function setPedFaceFeatures(ped, faceFeatures)
	if faceFeatures then
		for k, v in pairs(constants.FACE_FEATURES) do
			...
			if test2[test[k]] == nil or test2[test[k]] ~= faceFeatures[v] then
				test2[test[k]] = faceFeatures[v]
				print(test[k], faceFeatures[v])
			end
		end
	end
end
...
local function setPedHairAndDecorations(ped, hair, tattoos)
	...
	local hairDecoration = nil
	if hair then
		hairDecoration = getPedHairDecoration(ped, hair.style)
	end

	if pedTattoos then
		ClearPedDecorations(ped)
		for i = 1, #pedTattoos do
